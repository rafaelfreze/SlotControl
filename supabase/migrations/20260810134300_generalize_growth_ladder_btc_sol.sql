-- CoinOps unified BTC/SOL growth ladder.
--
-- This migration generalizes the audited BTC capital model to SOL without
-- rewriting historical counters, operational values, prices, positions or
-- legacy history. Existing BTC rows retain their identifiers and provenance.

create temporary table coinops_growth_ladder_equity_before as
select
  slot.id,
  slot.gains,
  slot.real_gains,
  ...
  slot.redistribution_received_usdt,
  slot.redistribution_sent_usdt,
  slot.status,
  slot.preco_entrada,
  slot.preco_atual,
  slot.preco_alvo
...
alter table coinops.btc_redistribution_batches
  add column if not exists asset text;

update coinops.btc_redistribution_batches
set asset = 'BTC'
where asset is null;

alter table coinops.btc_redistribution_batches
  alter column asset set default 'BTC',
  alter column asset set not null,
  drop constraint if exists btc_redistribution_batches_asset_check;

alter table coinops.btc_redistribution_batches
  add constraint btc_redistribution_batches_asset_check
  check (asset in ('BTC', 'SOL'));

alter table coinops.btc_redistribution_transfers
  drop constraint if exists btc_redistribution_transfers_asset_check;

alter table coinops.btc_redistribution_transfers
  add constraint btc_redistribution_transfers_asset_check
  check (asset in ('BTC', 'SOL'));
...
alter table coinops.btc_external_contributions
  alter column asset set default 'BTC',
  alter column asset set not null,
  drop constraint if exists btc_external_contributions_asset_check;

alter table coinops.btc_external_contributions
  add constraint btc_external_contributions_asset_check
  check (asset in ('BTC', 'SOL'));

alter table coinops.growth_plan_goal_audit
  drop constraint if exists growth_plan_goal_audit_asset_check;

alter table coinops.growth_plan_goal_audit
  add constraint growth_plan_goal_audit_asset_check
  check (asset in ('BTC', 'SOL'));
...
create unique index btc_redistribution_batches_completed_month_uidx
  on coinops.btc_redistribution_batches
    (product_id, tenant_id, user_id, asset, month_reference)
  where status = 'COMPLETED';

create index if not exists btc_redistribution_batches_scope_asset_created_idx
  on coinops.btc_redistribution_batches
    (product_id, tenant_id, user_id, asset, created_at desc);

create index if not exists btc_external_contributions_scope_asset_created_idx
  on coinops.btc_external_contributions
    (product_id, tenant_id, user_id, asset, created_at desc);

create or replace function private.coinops_normalize_growth_asset(p_asset text)
returns text
language plpgsql
immutable
strict
...
as $asset$
declare
  asset_code text := upper(btrim(p_asset));
begin
  if asset_code not in ('BTC', 'SOL') then
    raise exception 'COINOPS_GROWTH_ASSET_INVALID';
  end if;
  return asset_code;
end;
$asset$;
...
declare
  ...
  algorithm_version text := asset_code || '_LADDER_ASSISTED_V2';
...
begin
  ...
  loop
    ...
    if slot_record.operational_gains < 0 or slot_record.operational_slot_value < 0 then
      raise exception 'COINOPS_GROWTH_OPERATIONAL_STATE_INVALID_FOR_SLOT:%', slot_record.id;
    end if;
  ...
  loop
    ...
    loop
      ...
      if donor_operational_after < 0 or donor_value_after < 0 then
        raise exception 'COINOPS_GROWTH_DONOR_RESULT_INVALID:%', donor_record.slot_key;
      end if;
...
$preview$;
create or replace function coinops.get_asset_ladder_plan(p_asset text)
returns jsonb
language plpgsql
security definer
stable
...
declare
  ...
  display_reference_level numeric(20, 8);
  preview_data jsonb;
...
begin
  ...
  suggested_reference_level := round(plan_monthly_goal::numeric * plan_cycle_number, 8);
  ...
  display_reference_level := coalesce(plan_reference_level, suggested_reference_level);
  preview_data := private.coinops_build_asset_ladder_preview(
    scope_row.product_id,
    scope_row.tenant_id,
    scope_row.user_id,
    asset_code,
    display_reference_level
  );
  ...
  where ledger.product_id = scope_row.product_id
    ...
    and coalesce(ledger.metadata ->> 'asset', 'BTC') = asset_code
  ...
  return jsonb_build_object(
    ...
    'available_excess_gains', preview_data -> 'available_excess_gains',
    'available_excess_usdt', preview_data -> 'available_excess_usdt',
...
begin
  ...
  if exists (
    ...
    raise exception 'COINOPS_GROWTH_MONTH_ALREADY_COMPLETED';
  end if;
...
$prepare$;
create or replace function coinops.confirm_asset_ladder_redistribution(
  p_asset text,
  p_batch_id uuid,
  p_idempotency_key uuid
)
...
begin
  ...
  for update;
  if not found then
    raise exception 'COINOPS_GROWTH_BATCH_NOT_FOUND';
  end if;
  ...
  if batch_row.status <> 'PREPARED' then
    raise exception 'COINOPS_GROWTH_BATCH_NOT_PREPARED:%', batch_row.status;
  end if;
  if batch_row.expires_at <= timezone('utc', now()) then
    raise exception 'COINOPS_GROWTH_BATCH_EXPIRED';
  end if;
  if batch_row.transfer_count <= 0 or batch_row.total_transferred_usdt <= 0 then
    raise exception 'COINOPS_GROWTH_BATCH_EMPTY';
  end if;
  ...
  recomputed_preview := private.coinops_build_asset_ladder_preview(
    scope_row.product_id,
    scope_row.tenant_id,
    scope_row.user_id,
    asset_code,
  ...
  if recomputed_preview ->> 'algorithm_version' <> batch_row.algorithm_version
    ...
    raise exception 'COINOPS_GROWTH_BATCH_STALE';
  end if;
  if (recomputed_preview ->> 'equity_difference_usdt')::numeric <> 0
    or batch_row.equity_difference <> 0 then
    raise exception 'COINOPS_GROWTH_CONSERVATION_FAILED';
  end if;
  ...
  if persisted_transfer_count <> batch_row.transfer_count
    ...
    raise exception 'COINOPS_GROWTH_BATCH_TRANSFER_SET_INVALID';
  end if;
  ...
  if actual_equity_before <> batch_row.equity_before then
    raise exception 'COINOPS_GROWTH_BATCH_EQUITY_CHANGED';
  end if;
  ...
  loop
    ...
    if donor_before.status <> transfer_row.donor_status
      ...
      raise exception 'COINOPS_GROWTH_TRANSFER_SNAPSHOT_MISMATCH:%', transfer_row.sequence_number;
    end if;
    ...
    if donor_after.operational_slot_value <> transfer_row.donor_value_after
      ...
      raise exception 'COINOPS_GROWTH_TRANSFER_POSTCONDITION_FAILED:%', transfer_row.sequence_number;
    end if;
  ...
  if actual_equity_difference <> 0
    ...
    raise exception 'COINOPS_GROWTH_CONFIRM_EQUITY_MISMATCH:%', actual_equity_difference;
  end if;
...
$confirm$;
create or replace function coinops.cancel_asset_ladder_redistribution(
  p_asset text,
  p_batch_id uuid
)
returns jsonb
...
begin
  ...
  if batch_row.status <> 'PREPARED' then
    raise exception 'COINOPS_GROWTH_BATCH_NOT_CANCELLABLE:%', batch_row.status;
  end if;
...
begin
  ...
  for update of slot;
  if not found then
    raise exception 'COINOPS_GROWTH_SLOT_NOT_FOUND';
  end if;
...
begin
  ...
  result := coinops.apply_asset_external_contribution(
    asset_code,
    p_slot_id,
    candidate_amount,
    normalized_reason,
...
create or replace function coinops.update_asset_strategy(
  p_strategy_id uuid,
  p_title text,
...
begin
  if p_strategy_id is null or char_length(normalized_title) not between 1 and 120 then
    raise exception 'COINOPS_GROWTH_STRATEGY_ID_OR_TITLE_INVALID';
  end if;
  if normalized_base_value is null or normalized_base_value <= 0
    ...
    raise exception 'COINOPS_GROWTH_STRATEGY_VALUES_INVALID';
  end if;
  select * into strict scope_row from private.coinops_current_scope();

  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(
    'coinops:asset-strategy:' || scope_row.product_id::text || ':'
      || scope_row.tenant_id::text || ':' || scope_row.user_id::text
      || ':' || p_strategy_id::text,
    0
  ));
  ...
  for update;
  if not found then
    raise exception 'COINOPS_GROWTH_STRATEGY_NOT_FOUND';
  end if;
  asset_code := strategy_before.asset;
  ...
  if position_snapshot_after is distinct from position_snapshot_before then
    raise exception 'COINOPS_GROWTH_STRATEGY_SYNC_CHANGED_SLOT_STATE';
  end if;
  if exists (
    select 1
    from coinops.slots slot
    ...
    raise exception 'COINOPS_GROWTH_STRATEGY_RATE_SYNC_FAILED';
  end if;
  ...
  insert into coinops.history_events (
    ...
    strategy_after.id, 'Estrategia', jsonb_build_object(
      'schemaVersion', 1,
      'eventType', lower(asset_code) || '_strategy_updated_atomically',
      'asset', asset_code,
...
$gain_breakdown$;
create or replace function private.coinops_protect_growth_strategy()
returns trigger
language plpgsql
set search_path = ''
as $strategy_asset$
...
begin
  ...
  if old.asset in ('BTC', 'SOL')
    ...
    raise exception 'COINOPS_GROWTH_STRATEGY_CAPITAL_CONFIG_REQUIRES_RPC';
  end if;
  return new;
end;
$strategy_asset$;
...
$ledger$;
create or replace function private.coinops_audit_growth_goals()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if tg_op = 'INSERT' or new.sol_monthly_goal is distinct from old.sol_monthly_goal then
    insert into coinops.growth_plan_goal_audit (
      product_code, product_id, tenant_id, user_id, asset,
      previous_goal, new_goal, changed_by
    ) values (
      'coinops', new.product_id, new.tenant_id, new.user_id, 'SOL',
      case when tg_op = 'INSERT' then null else old.sol_monthly_goal end,
      new.sol_monthly_goal,
      coalesce((select auth.uid()), new.user_id)
    );
  end if;
  return new;
...
-- Preserve the currently executed SOL positions before enabling the same
-- snapshot-based accounting already used by BTC. Historical entry/target,
-- status and gain counters are intentionally left untouched.
update coinops.slots slot
set
  position_notional_usdt = coalesce(slot.position_notional_usdt, slot.operational_slot_value),
  position_gain_unit_usdt = coalesce(
    slot.position_gain_unit_usdt,
    private.coinops_gain_unit_usdt(slot.base_value, slot.growth_contribution, slot.gain_rate)
  ),
  position_quantity = coalesce(
    slot.position_quantity,
    case
      when slot.preco_entrada is not null and slot.preco_entrada > 0 then
        round(slot.operational_slot_value / slot.preco_entrada, 16)
      else null
    end
  ),
...
where strategy.product_id = slot.product_id
  ...
  and strategy.user_id = slot.user_id
  and strategy.id = slot.strategy_id
  and strategy.asset = 'SOL'
  and slot.status = 'aberto'
  and (
    slot.position_notional_usdt is null
    or slot.position_gain_unit_usdt is null
    or slot.position_opened_at is null
  );

-- SOL receives an additive opening ledger at cutover. This records the
-- existing state as-is and never reclassifies legacy added gains.
insert into coinops.slot_capital_ledger (
  product_code, product_id, tenant_id, user_id, slot_id, entry_type,
  amount_usdt, operational_gain_delta, operational_before, operational_after,
  value_before, value_after, gain_unit_before_usdt, gain_unit_after_usdt,
...
join coinops.strategies strategy
 ...
 and strategy.user_id = slot.user_id
 and strategy.id = slot.strategy_id
where strategy.asset = 'SOL'
on conflict do nothing;

-- The same immutable accounting rules now protect both growth assets.
...
for each row execute function private.coinops_apply_asset_realized_profit_on_real_gain();

drop trigger if exists slots_capture_btc_position_snapshot on coinops.slots;
drop trigger if exists slots_capture_asset_position_snapshot on coinops.slots;
create trigger slots_capture_asset_position_snapshot
before insert or update of status, preco_entrada, base_value, gain_rate on coinops.slots
for each row execute function private.coinops_capture_asset_position_snapshot();
...
on coinops.slots
for each row execute function private.coinops_enforce_asset_gain_breakdown();
...
for each row execute function private.coinops_protect_growth_strategy();

drop trigger if exists slots_record_btc_real_gain on coinops.slots;
drop trigger if exists slots_record_asset_real_gain on coinops.slots;
create trigger slots_record_asset_real_gain
after update of gains, real_gains, status on coinops.slots
for each row execute function private.coinops_record_asset_real_gain();

drop trigger if exists growth_plan_settings_audit_btc_goal on coinops.growth_plan_settings;
drop trigger if exists growth_plan_settings_audit_growth_goals on coinops.growth_plan_settings;
create trigger growth_plan_settings_audit_growth_goals
after insert or update of btc_monthly_goal, sol_monthly_goal on coinops.growth_plan_settings
for each row execute function private.coinops_audit_growth_goals();

comment on table coinops.btc_redistribution_batches is
  'Audited BTC/SOL growth-ladder batches. Physical name retained for backward compatibility.';
comment on table coinops.btc_redistribution_transfers is
  'Audited BTC/SOL growth-ladder transfers. Physical name retained for backward compatibility.';
comment on table coinops.btc_external_contributions is
  'Audited BTC/SOL external contributions and manual operational-gain additions. Physical name retained for backward compatibility.';

revoke all on function private.coinops_normalize_growth_asset(text) from public, anon, authenticated, service_role;
revoke all on function private.coinops_build_asset_ladder_preview(uuid, uuid, uuid, text, numeric) from public, anon, authenticated, service_role;
revoke all on function private.coinops_capture_asset_position_snapshot() from public, anon, authenticated, service_role;
revoke all on function private.coinops_apply_asset_realized_profit_on_real_gain() from public, anon, authenticated, service_role;
revoke all on function private.coinops_enforce_asset_gain_breakdown() from public, anon, authenticated, service_role;
revoke all on function private.coinops_protect_growth_strategy() from public, anon, authenticated, service_role;
revoke all on function private.coinops_record_asset_real_gain() from public, anon, authenticated, service_role;
revoke all on function private.coinops_audit_growth_goals() from public, anon, authenticated, service_role;

revoke all on function coinops.get_asset_ladder_plan(text) from public, anon, authenticated, service_role;
revoke all on function coinops.prepare_asset_ladder_redistribution(text, numeric, uuid) from public, anon, authenticated, service_role;
revoke all on function coinops.confirm_asset_ladder_redistribution(text, uuid, uuid) from public, anon, authenticated, service_role;
revoke all on function coinops.cancel_asset_ladder_redistribution(text, uuid) from public, anon, authenticated, service_role;
revoke all on function coinops.apply_asset_external_contribution(text, uuid, numeric, text, uuid) from public, anon, authenticated, service_role;
revoke all on function coinops.apply_asset_manual_operational_gains(text, uuid, numeric, text, uuid) from public, anon, authenticated, service_role;
revoke all on function coinops.update_asset_strategy(uuid, text, numeric, numeric, numeric, integer) from public, anon, authenticated, service_role;

grant execute on function coinops.get_asset_ladder_plan(text) to authenticated;
grant execute on function coinops.prepare_asset_ladder_redistribution(text, numeric, uuid) to authenticated;
grant execute on function coinops.confirm_asset_ladder_redistribution(text, uuid, uuid) to authenticated;
grant execute on function coinops.cancel_asset_ladder_redistribution(text, uuid) to authenticated;
grant execute on function coinops.apply_asset_external_contribution(text, uuid, numeric, text, uuid) to authenticated;
grant execute on function coinops.apply_asset_manual_operational_gains(text, uuid, numeric, text, uuid) to authenticated;
grant execute on function coinops.update_asset_strategy(uuid, text, numeric, numeric, numeric, integer) to authenticated;

-- Migration safety gate: no historical or financial slot field may change.
do $preservation$
begin
  if exists (
    select 1
    from coinops_growth_ladder_equity_before before_state
    join coinops.slots slot on slot.id = before_state.id
    where slot.gains is distinct from before_state.gains
       or slot.real_gains is distinct from before_state.real_gains
       or slot.added_gains is distinct from before_state.added_gains
       or slot.operational_gains is distinct from before_state.operational_gains
       or slot.base_value is distinct from before_state.base_value
       or slot.realized_profit is distinct from before_state.realized_profit
       or slot.growth_contribution is distinct from before_state.growth_contribution
       or slot.operational_slot_value is distinct from before_state.operational_slot_value
       or slot.redistribution_received_usdt is distinct from before_state.redistribution_received_usdt
       or slot.redistribution_sent_usdt is distinct from before_state.redistribution_sent_usdt
       or slot.status is distinct from before_state.status
       or slot.preco_entrada is distinct from before_state.preco_entrada
       or slot.preco_atual is distinct from before_state.preco_atual
       or slot.preco_alvo is distinct from before_state.preco_alvo
  ) then
    raise exception 'COINOPS_GROWTH_MIGRATION_CHANGED_EXISTING_FINANCIAL_STATE';
  end if;
end;
$preservation$;

drop table coinops_growth_ladder_equity_before;
