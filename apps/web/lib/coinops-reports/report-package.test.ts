...
test("pacote v6 contém REGIME_ATH e AJUSTES_MANUAIS com contagens e hashes", () => {
  ...
  assert.equal(files.size, 23); assert.equal(manifest.report_version, 6); assert.equal(manifest.timezone, "America/Campo_Grande"); assert.equal(manifest.app_commit_sha, "abc123");
  assert.equal(manifest.schema_version, "coinops/robot-v1/reports-v6"); assert.ok(files.has("16_MISSED_TEMPORAL.csv")); assert.ok(files.has("17_METAS_MENSAIS.csv")); assert.ok(files.has("REGIME_ATH.csv")); assert.ok(files.has("AJUSTES_MANUAIS.csv"));
