...
test("pacote v5 contém REGIME_ATH, contagens e hashes correspondem ao conteúdo descompactado", () => {
  ...
  assert.equal(files.size, 22); assert.equal(manifest.report_version, 5); assert.equal(manifest.timezone, "America/Campo_Grande"); assert.equal(manifest.app_commit_sha, "abc123");
  assert.equal(manifest.schema_version, "coinops/robot-v1/reports-v5"); assert.ok(files.has("16_MISSED_TEMPORAL.csv")); assert.ok(files.has("17_METAS_MENSAIS.csv")); assert.ok(files.has("REGIME_ATH.csv"));
