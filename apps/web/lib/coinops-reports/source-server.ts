...
export async function loadRawReportSources(filters: ReportFilters): Promise<RawReportSources> {
  ...
  const [configs, cycles, runs] = await Promise.all([
    ...
    withShadow ? load("robot_v1_cycles", source("id,strategy_version,config_id,asset,symbol,execution_mode,status,anchor_price,slot_notional_usdc,capital_usdc,gain_rate,entry_spacing,completion_reason,started_at,completed_at,created_at,updated_at", ["started_at", "id"], { assetColumn: "asset", time: "started_at", until })) : Promise.resolve([]),
    withTestnet ? load("robot_v1_testnet_runs", source("id,strategy_version,asset,symbol,status,anchor_price,slot_notional_usdc,gain_rate,entry_spacing,next_capital_usdc,next_gain_rate,next_entry_spacing,last_reconciled_at,last_error,created_at,updated_at,previous_run_id,completed_at,completion_reason,terminal_fill_client_order_id,reset_idempotency_key,reset_started_at,reset_completed_at,recovery_source", ["created_at", "id"], { assetColumn: "asset", time: "created_at", until })) : Promise.resolve([]),
  ]);
  ...
  if (withShadow) tasks.push(
    () => load("robot_v1_slots", source("id,cycle_id,slot_number,logical_level,operation_sequence,symbol,allocation_usdc,entry_state,armed_at,missed_at,buy_price,requested_quantity,executed_quantity,average_fill_price,buy_status,take_profit_price,take_profit_status,status,buy_client_order_id,sell_client_order_id,buy_exchange_order_id,sell_exchange_order_id,buy_fee,sell_fee,realized_quote_pnl,idempotency_key,observed_at,buy_trigger_price,buy_trigger_observed_price,buy_triggered_at,tp_trigger_observed_price,tp_triggered_at,created_at,updated_at", ["cycle_id", "slot_number"], { related: relatedCycles })),
    ...
    () => load("robot_v1_slot_operations", source("id,cycle_id,slot_id,physical_slot_number,logical_level,operation_sequence,symbol,allocation_usdc,entry_price,executed_quantity,take_profit_price,buy_client_order_id,sell_client_order_id,opened_at,closed_at,gross_quote_pnl,estimated_quote_fees,net_quote_pnl,created_at", ["closed_at", "id"], { related: relatedCycles })),
