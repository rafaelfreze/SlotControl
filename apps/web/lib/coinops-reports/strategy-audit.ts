...
export function buildStrategyAuditChecks(data: AuditDatasets, context: Context): AuditRow[] {
  ...
  for (const environment of context.filters.environments.filter((value) => value !== "REAL")) for (const asset of context.filters.assets) {
    ...
    add("STRATEGY_DECISION_DISPATCH", undispatched.length || failures.length ? "FAIL" : missing || !decisions.length || pendingDispatch ? "WARNING" : "PASS", `${undispatched.length} decisão(ões) sem despacho original após 120s; ${recoveredUndispatched.length} já recuperada(s) por pós-condição comprovada, sem apagar a falha histórica. ${undispatched.length - recoveredUndispatched.length} ainda não recuperada(s); ${failures.length} falha(s) persistida(s). Pendências recentes não são sucesso.`, { ...scope, active_failures: undispatched.length - recoveredUndispatched.length + failures.length, recovered_historical_failures: recoveredUndispatched.length });
