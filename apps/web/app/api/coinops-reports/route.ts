...
const CANDLE_COLUMNS: ExportColumn[] = ["operator_id", "exchange_account_id", "account_display_name", "trading_engine_id", "environment", "quote_asset", "symbol", "candle_open_at", "candle_close_at", "open_price", "high_price", "low_price", "close_price", "source", "created_at"].map((key) => ({ key, label: key }));
...
export async function GET(request: Request) {
  try {
    ...
    if (format === "preview") {
      return NextResponse.json(sanitizeExportValue({ reportVersion: REPORT_VERSION, timezone: REPORT_TIMEZONE, generatedAt: source.generatedAt, filters, summaries: report.datasets.summary, totals: { errors: report.datasets.alerts.filter((row) => row.severity === "ERROR").length }, checks: report.datasets.checks, warnings: report.warnings, incompleteSources: report.incompleteSources, rowCounts: packaged.manifest.row_counts, files: packaged.files.map((file) => ({ name: file.name, bytes: Buffer.byteLength(file.content), rows: packaged.manifest.row_counts[file.name] ?? null, description: REPORT_FILES.find((definition) => definition.name === file.name)?.description || file.name })), safety: { report_access: "READ_ONLY", live: "INDEPENDENT_SERVER_SIDE_RUNTIME", writes_from_report: 0 } }), { headers: PRIVATE_HEADERS });
    }
