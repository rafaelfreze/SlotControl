...
async function planPreview(scope: Scope, input: EnginePlanInput) {
  ...
  const snapshot = await operatorAccountSnapshot(scope.operator.id, plan.accountId, plan.quote,
    plan.engines.map((engine) => engine.symbol));
  ...
  return { account: account.display_name, accountId: account.id, quote: plan.quote,
...
export async function GET() {
  try {
    ...
    const [accounts, engines, caps, runs, profiles, checks, preparations, alerts] = await Promise.all([
      ...
      service.from("robot_v1_ath_profiles")
        ...
        .eq("operator_id", operator.id).eq("environment", "REAL"),
    ...
    if ([accounts, engines, caps, runs, profiles, checks, preparations, alerts].some((item) => item.error))
    ...
    if (slots.error || orders.error) throw new Error("COINOPS_ENGINE_STATUS_UNAVAILABLE");
    const validation = new Map<string, boolean>();
    for (const row of checks.data ?? []) if (!validation.has(row.exchange_account_id))
      validation.set(row.exchange_account_id, row.status === "PASS" && row.evidence?.status === "PASS");
    const accountById = new Map((accounts.data ?? []).map((item) => [item.id, item]));
    return json({ accounts: (accounts.data ?? []).map((item) => ({ ...item,
      credentialValidated: validation.get(item.id) ?? false })),
      engines: (engines.data ?? []).map((item) => {
        const run = runs.data?.find((row) => row.trading_engine_id === item.id) ?? null;
        const runSlots = (slots.data ?? []).filter((row) => row.run_id === run?.id);
        const runOrders = (orders.data ?? []).filter((row) => row.run_id === run?.id);
        ...
        const operational = run?.status === "ACTIVE" && item.status === "ACTIVE" && !item.kill_switch
          && account?.status === "ACTIVE" && !account.kill_switch && prep?.live_enabled && !prep.kill_switch
          && clean && recent && runSlots.length === 25 && open > 0 && tp === open && nextBuy === 1;
        const ready = run?.status === "PREPARING" && !run.last_error && runSlots.length === 25
  ...
  try {
    ...
    if (input.action === "PREVIEW" || input.action === "PROVISION") {
      ...
      await syncInactiveBinanceAccount(scope.service, scope.operator.id, plan.accountId,
        `account_${plan.accountId.replaceAll("-", "")}`, "REAL");
    ...
    if (input.action === "SYNC") {
      ...
      const result = await syncInactiveBinanceAccount(scope.service, scope.operator.id,
        account.id, account.credential_ref!, "REAL");
      return json(result);
    }
    ...
    const engine = await resolveOperatorEngine(scope.service, scope.operator, {
      environment: "REAL", exchange_account_id: input.accountId,
