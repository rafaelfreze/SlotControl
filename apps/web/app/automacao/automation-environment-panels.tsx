...
export function EnvironmentConnectionStrip({ data, environment, asset }: { data: Props; environment: "TESTNET" | "REAL"; asset: "BTC" | "SOL" }) {
  ...
  return <section className="av2-connection-panel"><div><strong>{error ? "Testnet · atenção" : data.testnetRun?.status === "ACTIVE" ? "TESTNET OPERANDO · fundos fictícios" : data.testnet?.ok ? "TESTNET CONECTADO · fundos fictícios" : "Testnet · verificação pendente"}</strong><small>{error || "Ordens exclusivas da Binance Spot Testnet. Production permanece READ-ONLY."}</small></div><div><strong>Reconciliação Testnet</strong><small>{when(data.testnetRun?.last_reconciled_at)} · {result.missedLevels} missed levels · LIVE bloqueado.</small></div><div><strong>Resultado Testnet total</strong><small>{hasLedger ? `Realizado ${signed(result.realizedProfit)} + P&L aberto ${signed(result.openPnl)} USDC · capital comprometido ${amount(result.committedCapital, 4)} / livre ${amount(result.freeCapital, 4)} USDC` : "Ledger indisponível para totalização."}</small></div></section>;
}
