...
export type AdjustmentPreviewResult = { snapshot: AdjustmentSnapshot; preview: AdjustmentPreview };

type Service = ReturnType<typeof createServiceRoleClient>;
type Scope = { productId: string; tenantId: string; userId: string; service: Service };
...
async function loadSnapshot(ctx: Scope, environment: AdjustmentEnvironment, asset: Asset,
  ...
  if (environment === "SHADOW") {
    const { data: config, error: configError } = await base.from("robot_v1_configs")
      ...
      .eq("user_id", ctx.userId).eq("asset", asset).eq("execution_mode", "SHADOW").single();
    ...
    const { data: run, error: runError } = await base.from("robot_v1_testnet_runs")
      .select("id,gain_rate").eq("product_id", ctx.productId).eq("tenant_id", ctx.tenantId)
      .eq("user_id", ctx.userId).eq("asset", asset).eq("status", "ACTIVE").single();
  ...
  const { data: total, error: totalError } = await base.from("robot_v1_slot_gain_totals")
    ...
    .eq("environment", environment).eq("asset", asset).eq("slot_number", slotNumber).maybeSingle();
  if (totalError) throw new Error("COINOPS_ADJUSTMENT_GAIN_LEDGER_UNAVAILABLE");
  return { environment, asset, physicalSlotNumber: slotNumber, balanceUsdc,
...
export async function getManualAdjustmentFxQuote(): Promise<FxQuote> {
  await scope();
  const response = await fetch("https://data-api.binance.vision/api/v3/ticker/24hr?symbol=USDCBRL",
  ...
  if (ticker.symbol !== "USDCBRL" || !Number.isFinite(Number(ticker.closeTime)))
    throw new Error("COINOPS_ADJUSTMENT_FX_UNAVAILABLE");
  return validateFxQuote({ source: FX_SOURCE, rateBrlPerUsdc: Number(ticker.askPrice),
...
async function buildPreview(ctx: Scope, draft: AdjustmentDraft) {
  ...
  const fx = draft.kind === "MANUAL_CONTRIBUTION" && draft.currency === "BRL"
    ? await getManualAdjustmentFxQuote() : null;
  return { snapshot, preview: previewManualAdjustment(snapshot, draft, fx) };
}
...
export async function confirmCoinOpsManualAdjustment(draft: AdjustmentDraft,
  ...
  if (current.snapshot.balanceUsdc !== expected.snapshot.balanceUsdc
  ...
  const { data, error } = await ctx.service.rpc("apply_robot_v1_manual_adjustment", {
    ...
    p_environment: draft.environment, p_asset: draft.asset, p_slot_number: draft.slotNumber, p_kind: draft.kind,
...
export async function reverseCoinOpsManualAdjustment(input: {
  adjustmentId: string; reason: string; idempotencyKey: string;
}, expected: { balanceUsdc: number; monthlyGainCount: number; lifetimeGainCount: number }): Promise<{ id: string }> {
  ...
  const { data, error } = await ctx.service.rpc("apply_robot_v1_manual_adjustment", {
    p_product_id: ctx.productId, p_tenant_id: ctx.tenantId, p_user_id: ctx.userId, p_created_by: ctx.userId,
    p_environment: original.environment, p_asset: original.asset, p_slot_number: original.slot_number,
...
export async function previewCoinOpsManualReversal(adjustmentId: string) {
  ...
  return { adjustmentId, snapshot, deltaUsdc: delta, balanceAfterUsdc: snapshot.balanceUsdc + delta,
