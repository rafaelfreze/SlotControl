...
export function ManualAdjustmentsPanel({ recent, initial, expanded = false, context }: {
  context?: EngineContext; recent: RecentManualAdjustment[]; initial?: { environment: AdjustmentEnvironment; asset: "BTC" | "SOL"; slotNumber: number } | null; expanded?: boolean;
}) {
  const router = useRouter();
  const quote = context?.quote_asset ?? "USDC";
  const [environment, setEnvironment] = useState<AdjustmentEnvironment>(context?.environment ?? (initial?.environment === "REAL" ? "SHADOW" : initial?.environment ?? "SHADOW"));
  const [asset, setAsset] = useState<"BTC" | "SOL">((context?.base_asset as "BTC" | "SOL") ?? initial?.asset ?? "BTC");
  ...
  const makeDraft = (): AdjustmentDraft => ({ exchange_account_id: context?.exchange_account_id ?? "", trading_engine_id: context?.trading_engine_id ?? "", environment, asset, slotNumber, kind,
  ...
  const runConfirm = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setDone(`Ajuste ${result.id.slice(0, 8)} registrado. Novo saldo: ${money(result.balanceAfterUsdc)} ${quote}.`);
        router.refresh();
  ...
  const runReversePreview = (id: string) => {
    ...
    startTransition(async () => {
      try { setReversePreview(await previewCoinOpsManualReversal(id, { exchange_account_id: context?.exchange_account_id ?? "", trading_engine_id: context?.trading_engine_id ?? "", environment, asset })); }
  ...
  const runReverse = () => {
    ...
    startTransition(async () => {
      try {
        const result = await reverseCoinOpsManualAdjustment({ exchange_account_id: context?.exchange_account_id ?? "", trading_engine_id: context?.trading_engine_id ?? "", environment, asset, adjustmentId: reversePreview.adjustmentId,
          reason: reverseReason, idempotencyKey: reverseKey }, { ...reversePreview.snapshot, ...reversePreview.engine });
  ...
  if (context && (context.environment === "REAL" || !["USDC", "USDT"].includes(context.quote_asset))) return <p role="status">Ajustes de {context.account_display_name} · {context.symbol}: indisponíveis neste ledger nativo. Nenhum crédito é convertido ou aplicado implicitamente.</p>;
  return <details id="manual-adjustments" className="ma-panel" open={expanded || Boolean(initial) || undefined}>
    ...
    <div className="ma-body">{context ? <p className="ma-notice"><strong>{context.account_display_name} · {context.symbol} · {context.environment} · {context.quote_asset}</strong></p> : null}
      <p className="ma-notice">Ajuste de ledger, não é trade. A moeda do ledger é {quote}. O antigo ajuste Real em USDC está bloqueado; a preparação LIVE usa BRL separado, sem movimentação financeira.</p>
      <div className="ma-grid">
        <label>Ambiente<select disabled={Boolean(context)} value={environment} onChange={(event) => { setEnvironment(event.target.value as AdjustmentEnvironment); invalidate(); }}><option value="SHADOW">Shadow virtual</option><option value="TESTNET">Testnet fictício</option></select></label>
        <label>Ativo<select disabled={Boolean(context)} value={asset} onChange={(event) => { setAsset(event.target.value as "BTC" | "SOL"); invalidate(); }}><option value="BTC">BTC</option><option value="SOL">SOL</option></select></label>
        ...
        {kind === "MANUAL_TARGET_GAIN" ? <>
          ...
          <label>Moeda<select value={currency} onChange={(event) => { setCurrency(event.target.value as "USD" | "BRL"); invalidate(); }}><option value="USD">USD → saldo {quote}</option><option value="BRL">BRL → {quote} com câmbio real</option></select></label>
      ...
      {preview ? <div className="ma-preview" aria-live="polite"><strong>Preview · {context?.account_display_name} · {context?.symbol} · {context?.quote_asset} · {preview.snapshot.entryState} · Slot físico #{slotNumber}</strong>
        <div><span>Saldo</span><b>{money(preview.preview.balanceBeforeUsdc)} → {money(preview.preview.balanceAfterUsdc)} {quote}</b></div>
        <div><span>Crédito</span><b>+{money(preview.preview.convertedAmountUsdc, 8)} {quote}</b></div>
        ...
        {preview.preview.fx ? <div><span>Câmbio</span><b>R$ {money(preview.preview.originalAmount, 2)} ≈ {money(preview.preview.convertedAmountUsdc, 8)} {quote} · {money(preview.preview.fx.rateBrlPerUsdc, 6)} BRL/{quote} · Binance Spot {date(preview.preview.fx.observedAt)}</b></div> : null}
        {preview.preview.committedNotionalUsdc !== null ? <p>Posição OPEN comprometida: {money(preview.preview.committedNotionalUsdc)} {quote}. Entrada, quantidade, TP e ordem atuais não mudam. O saldo ajustado vale somente na próxima operação após o fechamento.</p>
      ...
      <details className="ma-history"><summary>Histórico recente e estornos ({recent.length})</summary>
        {recent.length ? <ol>{recent.map((row) => <li key={row.id}><span>{date(row.created_at)} · {row.environment} {row.asset} #{row.slot_number}<br /><strong>{row.kind === "MANUAL_TARGET_GAIN" ? `+${row.gain_units} gain manual` : row.kind === "MANUAL_CONTRIBUTION" ? "Aporte" : "Estorno"}</strong> · {money(row.converted_amount_usdc, 8)} {quote}{row.reversal_of ? ` · ref. ${row.reversal_of.slice(0, 8)}` : ""}</span>
        ...
        {reversePreview ? <div className="ma-preview"><strong>Preview do estorno · #{reversePreview.snapshot.physicalSlotNumber} {reversePreview.snapshot.asset}</strong>
          <div><span>Saldo</span><b>{money(reversePreview.snapshot.balanceUsdc)} → {money(reversePreview.balanceAfterUsdc)} {quote}</b></div>
