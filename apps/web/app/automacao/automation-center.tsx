...
function TestnetHero({ data }: { data: Props }) {
  ...
  return <section className="ac-panel ac-testnet-hero"><div className="ac-testnet-brand"><span className="ac-binance-mark" aria-hidden="true">✥</span><div><span className="ac-kicker">AMBIENTE FICTÍCIO</span><h2>Binance Spot Testnet <span className="ac-badge ac-badge--purple">{error ? "TESTNET — ERRO" : operating ? "TESTNET OPERANDO" : connected ? "TESTNET CONECTADO" : "AGUARDANDO VERIFICAÇÃO"}</span></h2><p>Fundos fictícios · Production permanece somente leitura.</p></div></div><div className="ac-permissions"><span className={connected ? "is-ok" : ""}>USER_DATA {connected ? "✓" : "—"}</span><span className={data.testnet?.ok && data.testnet.tradePermission.ok ? "is-ok" : ""}>TRADE {data.testnet?.ok && data.testnet.tradePermission.ok ? "✓" : "—"}</span><span className={data.testnet?.ok && data.testnet.userStreamPermission.ok ? "is-ok" : ""}>USER_STREAM {data.testnet?.ok && data.testnet.userStreamPermission.ok ? "✓" : "—"}</span></div><div className="ac-hero-action"><small>Última verificação<br /><strong>{data.testnet?.ok ? time(data.testnet.observedAt) : "Ainda não disponível"}</strong></small><a className="ac-button" href="/automacao?view=testnet&testnet=check">↻ &nbsp; Verificar agora</a></div>{error ? <p className="ac-error" role="alert">{error}</p> : null}</section>;
}
...
function TestnetEvents({ data }: { data: Props }) {
  ...
  return <section className="ac-panel ac-events"><div className="ac-panel-heading"><h2>Últimos eventos (Testnet)</h2><span>{data.testnetEvents.length} registros</span></div>{relevant.length ? <ol className="ac-timeline">{relevant.slice(0, 4).map((event, index) => <li key={`${event.observed_at}-${event.event_type}-${index}`}><span className="ac-event-dot" /><strong>{testnetEventLabel(event.event_type, event.slot_number)}</strong><time>{time(event.observed_at)}</time></li>)}</ol> : <p className="ac-empty">O ledger ainda não tem eventos Testnet.</p>}{data.testnetEvents.length > 4 ? <details className="ac-more-events"><summary>Ver histórico completo ({data.testnetEvents.length})</summary><ol className="ac-timeline">{data.testnetEvents.map((event, index) => <li key={`${event.observed_at}-${event.event_type}-${index}`}><span className="ac-event-dot" /><strong>{testnetEventLabel(event.event_type, event.slot_number)}</strong><time>{time(event.observed_at)}</time></li>)}</ol></details> : null}</section>;
}
...
export function AutomationDetails({ view, data, section, asset }: { view: AutomationView; data: Props; section: AutomationDetail; asset: "BTC" | "SOL" }) {
  ...
  else content = view === "overview" ? <Overview data={data} /> : view === "testnet" ? <Testnet data={data} /> : <>
    <div className="ac-production-balances" aria-label="Saldos Production somente leitura">{["USDT", "USDC", "BTC", "SOL"].map((asset) => {
      const balance = data.balances.find((item) => item.asset === asset);
      return <MetricCard key={asset} label={`Saldo ${asset}`} value={format(balance?.total, asset === "BTC" || asset === "SOL" ? 8 : 2)} note={`Disponível ${format(balance?.free, 8)} · bloqueado ${format(balance?.locked, 8)}`} />;
    })}</div><Live data={data} />
  </>;
