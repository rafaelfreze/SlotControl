...
function Testnet({ data: source, initialAsset }: { data: Props; initialAsset: "BTC" | "SOL" }) {
  ...
  const data = { ...source, ...selectTestnetAssetData(source, selectedAsset), testnetActionError: selectedAsset === "SOL" ? source.testnetActionError : null };
...
export function AutomationDetails({ view, data, section, asset }: { view: AutomationView; data: Props; section: AutomationDetail; asset: "BTC" | "SOL" }) {
  const scopedData = view === "testnet" ? { ...data, ...selectTestnetAssetData(data, asset), testnetActionError: asset === "SOL" ? data.testnetActionError : null } : data;
  let content;
