...
function TestnetHero({ data }: { data: Props }) {
  ...
  return <section className="ac-panel ac-testnet-hero"><div className="ac-testnet-brand"><span className="ac-binance-mark" aria-hidden="true">✥</span><div><span className="ac-kicker">AMBIENTE FICTÍCIO</span><h2>Binance Spot Testnet <span className="ac-badge ac-badge--purple">TESTNET {state}</span></h2><p>Fundos fictícios · Production permanece somente leitura.</p></div></div><div className="ac-permissions"><span className={connected ? "is-ok" : ""}>USER_DATA {connected ? "✓" : "—"}</span><span className={data.testnet?.ok && data.testnet.tradePermission.ok ? "is-ok" : ""}>TRADE {data.testnet?.ok && data.testnet.tradePermission.ok ? "✓" : "—"}</span><span className={data.testnet?.ok && data.testnet.userStreamPermission.ok ? "is-ok" : ""}>USER_STREAM {data.testnet?.ok && data.testnet.userStreamPermission.ok ? "✓" : "—"}</span></div><div className="ac-hero-action"><small>Última verificação<br /><strong>{data.testnet?.ok ? time(data.testnet.observedAt) : "Ainda não disponível"}</strong></small><a className="ac-button" href="/automacao?view=testnet&testnet=check">↻ &nbsp; Verificar agora</a></div>{error ? <p className="ac-error" role="alert">{error}</p> : null}</section>;
}

function BalanceCards({ data, asset }: { data: Props; asset?: "BTC" | "SOL" }) {
...
function TestnetConfiguration({ data, asset }: { data: Props; asset: "BTC" | "SOL" }) {
  ...
  return <section className="ac-panel ac-testnet-config"><div className="ac-panel-heading"><div><span className="ac-kicker">PERFIL DE TESTE 0,5% / 1%</span><h2>Configuração {asset}/USDC</h2></div><span className="ac-badge ac-badge--slate">25 SLOTS</span></div>
    <div className="ac-facts"><span>Ciclo atual <strong>{run?.status === "ACTIVE" ? "Ativo" : "Não iniciado"}</strong></span><span>Capital inicial <strong>{format(currentCapital, 2)} USDC</strong></span><span>Gain <strong>{format(gain, 2)}%</strong></span><span>Queda entre compras <strong>{format(spacing, 2)}%</strong></span><span>Iniciado em <strong>{time(run?.created_at)}</strong></span></div>
    {run?.status === "ACTIVE" ? <form className="av2-parameters" action={saveCoinOpsTestnetNextCycle} key={run.id}><input type="hidden" name="run_id" value={run.id} /><input type="hidden" name="asset" value={asset} /><h3>Próximo ciclo</h3><label>Capital (USDC)<input type="number" name="capital_usdc" min="0.01" max="2500" step="0.01" defaultValue={Number(run.next_capital_usdc ?? currentCapital)} required /></label><label>Gain %<input type="number" name="gain_percent" min="0.1" max="20" step="0.1" defaultValue={Number(run.next_gain_rate ?? run.gain_rate ?? 0.005) * 100} required /></label><label>Queda entre compras %<input type="number" name="spacing_percent" min="0.1" max="20" step="0.1" defaultValue={Number(run.next_entry_spacing ?? run.entry_spacing ?? 0.01) * 100} required /></label><button type="submit">Salvar próximo ciclo</button><button type="submit" name="preset" value="quick" formNoValidate>Usar perfil rápido 0,5% / 1%</button></form> : null}
    <small>Gain: percentual de alta necessário para vender uma posição. Queda entre compras: distância percentual entre entradas. Alterações valem apenas no próximo ciclo; posições e TPs atuais mantêm o snapshot original. LIVE permanece bloqueado.</small>
  </section>;
}

function ExecutionPanel({ data, asset = "SOL" }: { data: Props; asset?: "BTC" | "SOL" }) {
...
function OrdersTable({ data, asset = "SOL" }: { data: Props; asset?: "BTC" | "SOL" }) {
...
function MobileOrderCards({ orders, asset }: { orders: Order[]; asset: "BTC" | "SOL" }) {
  return <div className="ac-mobile-orders">{orders.map((order) => <article key={order.client_order_id}>
    ...
    <div className="ac-mobile-order-values"><span>Preço<strong>{orderPrice(order) ? `${format(orderPrice(order), 4)} USDC` : "Mercado"}</strong></span><span>Quantidade<strong>{format(orderQuantity(order), 6)} {asset}</strong></span></div>
...
function Testnet({ data: source, initialAsset }: { data: Props; initialAsset: "BTC" | "SOL" }) {
  ...
  const healthy = data.testnet?.ok && runState === "OPERANDO" && reconciliationAge < 15 * 60_000 && data.testnet?.tradePermission.ok && data.testnet.userStreamPermission.ok;
