...
const marketPrice = (data: Props, symbol: string) => Number(data.dailyCandles.filter((candle) => candle.symbol === symbol).at(-1)?.close_price || 0);
...
function MarketCard({ data, asset }: { data: Props; asset: "BTC" | "SOL" }) {
  ...
  const latest = Number(candles.at(-1)?.close_price || 0);
  const previous = Number(candles.at(-2)?.close_price || 0);
  const change = latest && previous ? (latest / previous - 1) * 100 : null;
  return <article className="ac-market"><div className="ac-market-title"><span className={`ac-coin ac-coin--${asset.toLowerCase()}`}>{asset === "BTC" ? "₿" : "≋"}</span><span><strong>{asset}/USDC</strong><small>Mercado à vista · consulta pública</small></span></div><strong className="ac-market-price">{latest ? format(latest, 2) : "Sem cotação"} <small>{latest ? "USDC" : ""}</small></strong><span className={change != null && change < 0 ? "ac-negative" : "ac-positive"}>{change == null ? "Variação indisponível" : `${change >= 0 ? "+" : ""}${format(change, 2)}% · diário`}</span><div className="ac-market-line"><CandleChart candles={candles} asset={asset} windowSize={14} /></div></article>;
}

function GlobalEvents({ data }: { data: Props }) {
  const events = [
    ...data.events.slice(0, 6).map((event) => ({ id: `shadow-${event.cycle_id}-${event.observed_at}-${event.event_type}`, at: event.observed_at, source: "SHADOW", label: shadowEventLabel(event.event_type) })),
    ...data.testnetEvents.slice(0, 6).map((event) => ({ id: `testnet-${event.observed_at}-${event.event_type}-${event.slot_number}`, at: event.observed_at, source: "TESTNET", label: testnetEventLabel(event.event_type, event.slot_number) }))
...
function testnetEventLabel(type: string, slot: number | null) {
  const labels: Record<string, string> = { RECONCILED: "Reconciliação concluída", BUY_NEW: "Próxima BUY armada", BUY_FILLED: "BUY preenchida", TP_NEW: "Take profit criado", TP_FILLED: "Take profit preenchido", BUY_CANCELED: "BUY própria cancelada", OWNED_BUY_REPLACED_AFTER_RESTART: "BUY substituída após recuperação", MISSED_LEVEL: "Nível atravessado" };
...
function BalanceCards({ data }: { data: Props }) {
  ...
  return <div className="ac-balance-grid">{assets.map((asset) => {
    ...
    return <article key={asset} className="ac-balance"><span className={`ac-coin ac-coin--${asset.toLowerCase()}`}>{asset === "BTC" ? "₿" : asset === "SOL" ? "≋" : asset === "USDT" ? "₮" : "$"}</span><div><span>Disponível Testnet ({asset})</span><strong>{balance ? format(balance.free, asset === "BTC" || asset === "SOL" ? 6 : 5) : "—"}</strong><small>{approx != null ? `≈ ${format(approx, 2)} USDC · última vela diária` : balance ? "Saldo fictício informado pela Binance" : "Verificação pendente"}</small></div></article>;
  })}</div>;
}

function ExecutionPanel({ data }: { data: Props }) {
  const orders = data.testnetOrders;
  const firstBuy = orders.find((order) => order.side === "BUY" && order.status === "FILLED");
  const residentTp = orders.find((order) => order.purpose === "TP" && openStatuses.has(order.status));
  ...
  return <section className="ac-panel ac-execution"><div className="ac-panel-heading"><div><span className="ac-kicker">SINGLE ACTIVE ENTRY</span><h2>Execução atual — {data.testnetRun?.symbol || "SOL/USDC"}</h2></div><span className="ac-badge ac-badge--purple">{data.testnetRun?.status === "ACTIVE" ? "CICLO ATIVO" : data.testnetRun?.status || "SEM CICLO"}</span></div><div className="ac-execution-rows">
    <ExecutionStep number="1" label={firstBuy ? `BUY · Slot #${firstBuy.slot_number}` : "BUY inicial"} state={firstBuy ? "Preenchida" : "Aguardando"} detail={firstBuy ? `${format(orderQuantity(firstBuy), 6)} SOL @ ${format(orderPrice(firstBuy), 2)} USDC` : "Nenhum fill registrado"} at={firstBuy?.created_at} tone="green" />
    <ExecutionStep number="2" label={residentTp ? `TAKE PROFIT · Slot #${residentTp.slot_number}` : "Take profit"} state={residentTp ? "Residente" : "Aguardando"} detail={residentTp ? `${format(orderQuantity(residentTp), 6)} SOL @ ${format(orderPrice(residentTp), 2)} USDC` : "Sem TP ativo"} at={residentTp?.created_at} tone="purple" />
