...
export function ShadowControls({ data, asset }: { data: Props; asset: "BTC" | "SOL" }) {
  ...
  return <section className="ac-panel premium-shadow-controls" aria-label={`Configuração Shadow ${asset}`}>
    <div className="ac-panel-heading"><div><span className="ac-kicker">SIMULAÇÃO VIRTUAL</span><h2>Shadow · {asset}/USDC</h2></div><span className={`ac-badge ${active ? "ac-badge--green" : "ac-badge--slate"}`}>{active ? "ATIVO" : config?.kill_switch ? "KILL SWITCH" : "ENTRADAS PAUSADAS"}</span></div>
    <p>Ciclo atual: capital {number(cycle?.capital_usdc ?? config?.capital_usdc, 2)} USDC · gain {percent(cycle?.gain_rate ?? config?.gain_rate)}% · queda entre compras {percent(cycle?.entry_spacing ?? config?.entry_spacing)}% · iniciado {date(cycle?.started_at)}.</p>
    ...
    <div className="av2-control-actions">
      ...
      <form action={controlRobotV1Shadow} onSubmit={confirmKill}><input type="hidden" name="asset" value={asset} /><input type="hidden" name="command" value="kill" /><button type="submit" className="av2-danger">Kill switch</button></form>
      <form action={controlRobotV1Shadow} className="av2-restart"><input type="hidden" name="asset" value={asset} /><input type="hidden" name="command" value="restart" /><label><input type="checkbox" name="restart_confirmed" value="yes" required /> Confirmo o reinício virtual e a preservação do histórico.</label><button type="submit">Reiniciar simulação</button></form>
    </div>
  </section>;
}
