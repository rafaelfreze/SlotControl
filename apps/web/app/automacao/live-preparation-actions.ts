...
import { actionEngine, requireLegacyRuntime } from "./engine-action-context";
...
export async function saveLiveAssetCaps(formData: FormData) {
  ...
  const engine = await actionEngine(formData, "REAL", undefined, { product_id: scope.productId, tenant_id: scope.tenantId, user_id: scope.userId });
  requireLegacyRuntime(engine);
  if (engine.quote_asset !== "BRL") throw new Error("COINOPS_LIVE_NATIVE_CAP_UNSUPPORTED");
  const asset = formData.get("asset");
  if (asset !== engine.base_asset || asset !== "BTC" && asset !== "SOL") throw new Error("COINOPS_LIVE_ASSET_INVALID");
  ...
  const { data: rows, error: readError } = await service.from("robot_v1_live_preparations")
    ...
    .eq("product_id", scope.productId).eq("tenant_id", scope.tenantId).eq("user_id", scope.userId).eq("exchange_account_id", engine.exchange_account_id);
  const { data: global, error: globalError } = await service.from("robot_v1_live_global_caps")
    .select("max_total_live_exposure_brl")
    .eq("product_id", scope.productId).eq("tenant_id", scope.tenantId).eq("user_id", scope.userId).eq("exchange_account_id", engine.exchange_account_id).single();
  ...
  const { data: saved, error } = await service.from("robot_v1_live_preparations").update({
    ...
    .eq("user_id", scope.userId).eq("exchange_account_id", engine.exchange_account_id).eq("quote_asset", "BRL").eq("updated_at", current.updated_at).select("id").maybeSingle();
