...
function eventLabel(event: Event) {
  const labels: Record<string, string> = { INITIAL_POSITION_OPENED: "Posição inicial aberta", BUY_TRIGGERED: "Compra Shadow executada", TP_TRIGGERED: "TP Shadow atingido", SLOT_PROFIT_CREDITED: "Lucro creditado no slot", SLOT_BALANCE_UPDATED: "Saldo do slot atualizado", SLOT_RECYCLED: "Slot reciclado", NEXT_BUY_ARMED: "Próxima compra armada", NEXT_BUY_DISARMED: "Próxima compra desarmada", MISSED_LEVEL_DURING_REARM: "Nível perdido no rearme", CYCLE_STARTED: "Novo ciclo", CYCLE_COMPLETED: "Ciclo concluído", CYCLE_RESTARTED: "Ciclo reiniciado", INTRABAR_AMBIGUOUS: "Vela ambígua", GRID_INVALID: "Grade inválida" };
  const number = event.next_state?.slotNumber;
  return `${labels[event.event_type] || event.event_type}${typeof number === "number" ? ` — Slot #${number}` : ""}`;
}
