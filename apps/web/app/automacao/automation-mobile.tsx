...
type Slot = { id: string; cycle_id: string; slot_number: number; buy_price: number | string; requested_quantity: number | string; buy_status: string; executed_quantity: number | string; average_fill_price: number | string | null; take_profit_price: number | string | null; take_profit_status: string; status: string; realized_quote_pnl: number | string | null };
      ...
      {(["BTC", "SOL"] as const).map((asset) => {
        ...
        return (
          <section className={`robot-shadow-card ${asset.toLowerCase()}`} key={asset}>
            ...
            <details className="robot-slots-details"><summary>Ver os {slots.length || 25} slots deste ciclo</summary>{slots.length ? <ol>{slots.map((slot) => { const view = slotPresentation(slot); return <li className={`robot-slot-${slot.status.toLowerCase()}`} key={slot.id}><span>#{slot.slot_number}</span><div><strong>{view.state}</strong>{view.badge ? <em>{view.badge}</em> : null}<small>{view.priceLabel}: {number(view.price, 4)}</small>{view.target !== null ? <small>Venda programada: {number(view.target, 4)} (+{percent(cycle?.gain_rate || config?.gain_rate)}%)</small> : null}<small>Valor: {number(Number(slot.requested_quantity) * Number(view.price || slot.buy_price), 2)} USDC</small>{slot.status === "CLOSED" ? <small>Resultado líquido: {number(slot.realized_quote_pnl, 4)} USDC</small> : null}</div></li>; })}</ol> : <p>Os slots aparecerão quando o ciclo Shadow iniciar.</p>}</details>
          </section>
        );
      })}
    </div>
