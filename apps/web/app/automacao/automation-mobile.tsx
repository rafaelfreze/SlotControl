        ...
        return (
          <section className={`robot-shadow-card ${asset.toLowerCase()}`} key={asset}>
            ...
            <div className="robot-control-grid">
              ...
              <form action={controlRobotV1Shadow} onSubmit={(event) => { if (!window.confirm("Reiniciar encerra somente este ciclo Shadow de teste, preserva seu histórico e inicia uma nova simulação. Continuar?")) event.preventDefault(); }}><input type="hidden" name="asset" value={asset} /><input type="hidden" name="command" value="restart" /><button type="submit">Reiniciar simulação</button></form>
            </div>
