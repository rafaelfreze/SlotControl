...
type Slot = { id: string; cycle_id: string; slot_number: number; logical_level: number; operation_sequence: number; allocation_usdc: number | string; buy_price: number | string; requested_quantity: number | string; executed_quantity: number | string; average_fill_price: number | string | null; take_profit_price: number | string | null; status: string; buy_triggered_at: string | null };
type Operation = { id: string; cycle_id: string; slot_id: string; physical_slot_number: number; logical_level: number; operation_sequence: number; allocation_usdc: number | string; entry_price: number | string; take_profit_price: number | string; gross_quote_pnl: number | string; estimated_quote_fees: number | string; net_quote_pnl: number | string; closed_at: string };
      ...
      return <section className={`robot-shadow-card ${asset.toLowerCase()}`} key={asset}>
        ...
        <details className="robot-slots-details"><summary>Ver os {slots.length || 25} slots — ordenados pelo nível lógico</summary>{ordered.length ? <ol>{ordered.map((slot) => {
          ...
          return <li className={`robot-slot-${slot.status.toLowerCase()}`} key={slot.id}><span>#{slot.slot_number}</span><div>
            ...
            <small>Saldo destinado {slot.status === "PENDING" ? "à próxima compra" : "à posição"}: {n(slot.allocation_usdc, 8)} USDC</small>
            <small>Quantidade virtual: {n(quantity, 8)} · notional executável: {n(notional, 8)} USDC</small>
            <small>Restante após filtro de lote: {n(Math.max(0, Number(slot.allocation_usdc) - notional), 8)} USDC</small>
            ...
            {history.length ? <details className="robot-operation-history"><summary>Histórico do slot físico</summary>{history.map((item) => <small key={item.id}>Ciclo {shortId(item.cycle_id)} · nível {item.logical_level} · saldo destinado {n(item.allocation_usdc, 8)} · BUY {n(item.entry_price, 4)} · TP {n(item.take_profit_price, 4)} · líquido {signed(item.net_quote_pnl)} USDC · {d(item.closed_at)}</small>)}</details> : null}
          </div></li>;
