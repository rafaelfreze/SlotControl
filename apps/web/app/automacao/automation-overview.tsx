...
function testnetAsset(data: Props, asset: Asset): TestnetAssetData | null {
  ...
  return selected.testnetRun ? { run: selected.testnetRun, slots: selected.testnetSlots, orders: selected.testnetOrders, events: selected.testnetEvents } : null;
}
...
const EVENT_LABELS: Record<string, string> = {
  INITIAL_POSITION_OPENED: "Posição inicial aberta", BUY_TRIGGERED: "Compra virtual executada", TP_TRIGGERED: "TP virtual atingido", SLOT_PROFIT_CREDITED: "Lucro creditado no slot", SLOT_BALANCE_UPDATED: "Saldo do slot atualizado", SLOT_RECYCLED: "Slot reciclado", NEXT_BUY_ARMED: "Próxima compra armada", NEXT_BUY_DISARMED: "Próxima compra desarmada", MISSED_LEVEL_DURING_REARM: "Nível perdido no rearme", CYCLE_STARTED: "Ciclo iniciado", CYCLE_COMPLETED: "Ciclo concluído", CYCLE_RESTARTED: "Ciclo reiniciado", INTRABAR_AMBIGUOUS: "Vela ambígua", GRID_INVALID: "Grade inválida", RECONCILED: "Reconciliação concluída", BUY_NEW: "Compra Testnet armada", BUY_FILLED: "Compra Testnet preenchida", SELL_NEW: "TP Testnet criado", SELL_FILLED: "TP Testnet preenchido", SLOT_CLOSED: "Slot concluído e ganho creditado", TP_PREPARED: "TP Testnet preparado", NEXT_BUY_PREPARED: "Próxima compra preparada", BUY_CANCELED: "Compra própria cancelada", OWNED_BUY_REPLACED_AFTER_RESTART: "Compra substituída na recuperação", MISSED_LEVEL: "Nível atravessado"
};
...
export function AutomationOverview({ data }: { data: Props }) {
  ...
  const testnetProfit = testnetKnown.length ? sum(testnetKnown.map((row) => row.result!.realizedProfit)) : null;
  const testnetGains = testnetKnown.length ? sum(testnetKnown.map((row) => row.result!.gains)) : null;
  ...
  return <div className="aov-overview">
    <div className="aov-environments">
      ...
      <section className="aov-environment aov-testnet" aria-labelledby="aov-testnet-title">
        ...
        <dl className="aov-metrics"><Metric label="Capital do robô" value={amount(testnetCapital)} unit="USDC" /><Metric label="Lucro realizado" value={signed(testnetProfit)} unit="USDC" positive={testnetProfit == null ? undefined : testnetProfit >= 0} /><Metric label="Gains acumulados" value={amount(testnetGains, 0)} /><Metric label="Operações concluídas" value={testnetKnown.length ? amount(testnetKnown.reduce((total, row) => total + row.result!.completedOperations, 0), 0) : "—"} /></dl>
        <div className="aov-assets">{testnet.map(({ asset, persisted, result }) => <article className="aov-asset" key={asset}><AssetHeading asset={asset} label={persisted?.run.symbol.replace(/(USDC|USDT|BRL)$/, "/$1") || `${asset}/USDC`} state={!persisted ? "Sem ciclo Testnet" : persisted.run.last_error ? "Revisar execução" : persisted.run.status === "ACTIVE" ? "Ciclo ativo" : "Ciclo registrado"} warning={Boolean(persisted?.run.last_error)} /><dl className="aov-asset-metrics"><Metric label="Capital" value={amount(result?.rows.length ? result.capital : null)} unit="USDC" /><Metric label="Lucro" value={signed(result?.rows.length ? result.realizedProfit : null)} positive={result?.rows.length ? result.realizedProfit >= 0 : undefined} /><Metric label="Gains" value={result?.rows.length ? amount(result.gains, 0) : "—"} /><Metric label="Ordens abertas" value={persisted ? String(persisted.orders.filter((order) => OPEN_ORDERS.has(order.status)).length) : "—"} /></dl><p className="aov-slot-state">{result?.rows.length ? <><strong>{result.openSlots}</strong> abertos <span>·</span> <strong>{result.armedSlots}</strong> próxima compra <span>·</span> <strong>{result.plannedSlots}</strong> planejados{result.missedLevels ? <em> · {result.missedLevels} níveis perdidos</em> : null}</> : "Sem saldo operacional persistido para este ativo"}</p></article>)}</div>
