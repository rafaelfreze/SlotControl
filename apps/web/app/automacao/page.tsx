...
type RobotV1ConfigRow = { id: string; asset: "BTC" | "SOL"; symbol: string; execution_mode: "SHADOW" | "TESTNET"; capital_usdc: number | string; slot_count: number; kill_switch: boolean };
type RobotV1CycleRow = { id: string; config_id: string; asset: "BTC" | "SOL"; status: string; slot_notional_usdc: number | string; started_at: string };
type RobotV1SlotRow = { id: string; cycle_id: string; slot_number: number; buy_price: number | string; buy_status: string; average_fill_price: number | string | null; take_profit_price: number | string | null; take_profit_status: string; status: string; realized_quote_pnl: number | string | null };
...
export default async function AutomationPage() {
  ...
  const [connectionResponse, engineResponse, assetResponse, intentsResponse, runsResponse, robotConfigsResponse, robotCyclesResponse, robotSlotsResponse] = await Promise.all([
    ...
    supabase.from("robot_v1_configs").select("id,asset,symbol,execution_mode,capital_usdc,slot_count,kill_switch").order("asset"),
    supabase.from("robot_v1_cycles").select("id,config_id,asset,status,slot_notional_usdc,started_at").in("status", ["STARTING", "GRID_ACTIVE", "POSITIONS_ACTIVE", "RESETTING"]),
    supabase.from("robot_v1_slots").select("id,cycle_id,slot_number,buy_price,buy_status,average_fill_price,take_profit_price,take_profit_status,status,realized_quote_pnl").order("slot_number")
  ]);
  ...
  return <DesktopWorkspace title="Automação / Exchange" subtitle="Binance Spot em leitura segura e CoinOps em Shadow" userLabel={user.email || "Usuário"}>
    ...
    <DesktopPanel eyebrow="ROBÔ V1" title="USDC isolado do manual USDT"><div className="desktop-kpi-grid">{(["BTC", "SOL"] as const).map((asset) => { const config = robotConfigs.find((row) => row.asset === asset); const cycle = config ? robotCycles.find((row) => row.config_id === config.id) : undefined; return <DesktopKpiCard key={asset} label={`${asset}/USDC`} value={config ? config.execution_mode : "PENDENTE"} helper={config ? `Capital ${displayNumber(config.capital_usdc, 2)} USDC · ${displayNumber(cycle?.slot_notional_usdc, 2)} por slot · ${config.kill_switch ? "kill switch ativo" : cycle?.status || "IDLE"}` : "Configure o capital no servidor antes de iniciar"} tone={config?.kill_switch ? "neutral" : "info"} />; })}</div><p className="desktop-helper">Production Binance: READ-ONLY · LIVE: BLOQUEADO · BTCUSDT/SOLUSDT: MANUAL PROTEGIDO · BTCUSDC/SOLUSDC: V1</p>{robotCycles.map((cycle) => { const slots = robotSlots.filter((row) => row.cycle_id === cycle.id); return <div className="desktop-table-wrap" key={cycle.id}><table><thead><tr><th>{cycle.asset}/USDC · {cycle.status}</th><th>BUY</th><th>Status BUY</th><th>Fill</th><th>TP</th><th>Status TP</th><th>Resultado</th></tr></thead><tbody>{slots.map((slot) => <tr key={slot.id}><td>#{slot.slot_number}</td><td>{displayNumber(slot.buy_price, 4)}</td><td>{slot.buy_status}</td><td>{displayNumber(slot.average_fill_price, 4)}</td><td>{displayNumber(slot.take_profit_price, 4)}</td><td>{slot.take_profit_status}</td><td>{displayNumber(slot.realized_quote_pnl, 4)}</td></tr>)}</tbody></table></div>; })}</DesktopPanel>
