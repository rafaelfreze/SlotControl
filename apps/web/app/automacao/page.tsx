...
export default async function AutomationPage() {
  ...
  const [connectionResponse, runsResponse, robotConfigsResponse, robotCyclesResponse, robotSlotsResponse, operationsResponse, eventsResponse, intentsResponse] = await Promise.all([
    ...
    supabase.from("robot_v1_slots").select("id,cycle_id,slot_number,logical_level,operation_sequence,buy_price,requested_quantity,buy_status,executed_quantity,average_fill_price,take_profit_price,take_profit_status,status,realized_quote_pnl,buy_triggered_at").order("slot_number"),
    supabase.from("robot_v1_slot_operations").select("id,cycle_id,slot_id,logical_level,operation_sequence,entry_price,take_profit_price,gross_quote_pnl,estimated_quote_fees,net_quote_pnl,opened_at,closed_at").order("closed_at", { ascending: false }),
