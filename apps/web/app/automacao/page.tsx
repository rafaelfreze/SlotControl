...
export default async function AutomationPage({ searchParams }: { searchParams?: { view?: string; testnet?: string; testnetError?: string } }) {
  ...
  await Promise.all((["BTC", "SOL"] as const).map(async (asset) => {
    ...
    const [slots, orders, events, missedEvents] = await Promise.all([
      ...
      supabase.from("robot_v1_testnet_events").select("run_id,event_type,slot_number,observed_at,details").eq("run_id", run.id).in("event_type", ["MISSED_LEVEL", "MISSED_LEVEL_DIAGNOSED"]).order("observed_at", { ascending: false }).limit(100)
    ]);
