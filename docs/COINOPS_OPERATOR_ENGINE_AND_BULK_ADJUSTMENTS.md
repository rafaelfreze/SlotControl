...
Na Testnet, uma falha de ativação antes da criação do ciclo pode deixar o motor `BLOQUEADO · SEM CICLO` com kill switch ligado. `Recuperar READY` no painel verifica novamente o gate READY, a ausência de qualquer ciclo/ordem daquele motor no ledger, um snapshot Binance Testnet sem ordens residentes no símbolo, saldo/cap/filtros e a permissão Spot via `/order/test` (sem criar ordem) antes de voltar o motor a INACTIVE/READY. A ação não envia ordens de trading nem altera Production. Se houver evidência ambígua, permanece bloqueado para investigação; não se deve repetir `Ativar` cegamente.
